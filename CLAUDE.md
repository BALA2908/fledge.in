...
## Current phase
Phases 1–6 built. Phase 6 (Communication Lab) code+content complete: 45 speaking prompts, lib/speech/metrics (60 tests total), /speak + /speak/[slug] recorder (Web Speech + MediaRecorder, audio never leaves device), dashboard Speaking card. BLOCKED on Bala applying migration 0002_speaking_prompt_slug.sql (adds slug col) → then `npm run db:seed` + smoke test. Phase 4 judge LIVE via self-hosted Piston (ephemeral tunnel — update PISTON_URL + add shared-secret for launch). Next: finish Phase 6 seed, then Phase 7 (hardening + launch)   ← update this line as you progress
